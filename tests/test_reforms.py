...
def test_income_tax_thresholds_match_budget():
    ...
    from scottish_budget_data.reforms import (
        INCOME_TAX_STARTER_THRESHOLD,
        INCOME_TAX_BASIC_THRESHOLD,
    )

    # Thresholds are stored as amounts above personal allowance (£12,570)
    # Starter rate (19%) ends at £16,537 total → £3,967 above PA
    # Basic rate (20%) ends at £29,526 total → £16,956 above PA
    PERSONAL_ALLOWANCE = 12_570

    assert INCOME_TAX_STARTER_THRESHOLD == 3_967
    assert INCOME_TAX_STARTER_THRESHOLD + PERSONAL_ALLOWANCE == 16_537

    assert INCOME_TAX_BASIC_THRESHOLD == 16_956
    assert INCOME_TAX_BASIC_THRESHOLD + PERSONAL_ALLOWANCE == 29_526
